...
2. **Detect the current project** from the working directory:
   ...
   - Pass this as the `project` parameter to boost same-project results and penalize cross-project noise
...
4. **For truncated results**, fetch the full document:
   ```
   Use mcp__cems__memory_get with:
...
2. Call `mcp__cems__memory_search` with query="hook development", max_results=5, project="Chocksy/cems"
...
2. Call `mcp__cems__memory_search` with query="typescript errors", max_results=10, scope="personal", project="<detected>"
3. Format and display results
